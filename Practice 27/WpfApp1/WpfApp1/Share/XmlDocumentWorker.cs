...
namespace WpfApp1.Share
{
    public class XmlDocumentWorker : IXmlWorker
    {
        ...
        public void Add(Country country)
        {
            ...
            XmlElement countryElem = _document.CreateElement("Сладость");
            XmlAttribute nameAttribute = _document.CreateAttribute("Тип");
            XmlText nameText = _document.CreateTextNode(country.Name);
            nameAttribute.AppendChild(nameText);

            XmlElement areaElem = _document.CreateElement("ТипОбертки");
            ...
            XmlElement populationElem = _document.CreateElement("Стоимость");
        ...
        public void Delete(string name)
        {
            ...
            foreach(XmlNode xnode in xRoot)
            {
                ...
                {
                    var attributeName = xnode.Attributes.GetNamedItem("Тип");
                    try
                    {
        ...
        public Country GetCountry(XmlNode node)
        {
            ...
            {
                var attributeName = node.Attributes.GetNamedItem("Тип");
            ...
            foreach(XmlNode childNode in node.ChildNodes)
            {
                try
                {
                    if (childNode.Name.Equals("Тип"))
                    {
                        country.Name = childNode.InnerText;
                    }
                    if (childNode.Name.Equals("ТипОбертки"))
                    {
                        country.Area = double.Parse(childNode.InnerText);
                    }
                    if (childNode.Name.Equals("Стоимость"))
                    {
